...
async function loadArchiveSource() {
  // This classifier reproduces the dated, reviewed source corpus. Fresh public
  // CMS exports already contain its later canonical titles and would remove
  // legacy duplicates before the historical merge/reject decisions are checked.
  // Restrict the snapshot to this bundle; the public archive keeps current CMS.
  const historicalCmsBytes = execFileSync("git", [
    "show",
    "d6c7849ccc5285e17c6da9ef147fecc9fdd0f000:src/data/cms/literaryWorks.generated.ts",
  ], { cwd: projectRoot, maxBuffer: 128 * 1024 });
  if (createHash("sha256").update(historicalCmsBytes).digest("hex") !==
    "6634bf7a357e7b1cfd1859d1194b66f355a8d6b1814da8342d9c75c3adcff2dd") {
    throw new Error("Historical enrichment CMS source fingerprint changed");
  }
  const historicalCmsSource = historicalCmsBytes.toString("utf8");
  ...
  if (!historicalCmsSource.startsWith(prefix) || !historicalCmsSource.endsWith(suffix)) {
    throw new Error("Historical enrichment CMS initializer changed");
  }
  const historicalCms = JSON.parse(historicalCmsSource.slice(prefix.length, -suffix.length));
  if (Object.keys(historicalCms).length !== 17 || Object.entries(historicalCms).some(([key, work]) =>
    key !== work.legacyId || key !== `${work.countryId}:${work.writerId}:${work.localId}`)) {
    throw new Error("Historical enrichment CMS identity set changed");
  }
  const cmsSourcePath = path.join(projectRoot, "src/data/cms/literaryWorks.generated.ts");
  let historicalCmsLoads = 0;
  ...
  try {
    await build({
      ...
      logLevel: "silent",
      plugins: [{
        name: "historical-enrichment-cms-source",
        setup(bundle) {
          bundle.onLoad({ filter: /[\\/]literaryWorks\.generated\.ts$/ }, (args) => {
            if (path.resolve(args.path) !== cmsSourcePath) return undefined;
            historicalCmsLoads += 1;
            return { contents: historicalCmsSource, loader: "ts" };
          });
        },
      }],
    });
    if (historicalCmsLoads !== 1) {
      throw new Error("Historical enrichment CMS source was not loaded exactly once");
    }
    return await import(
