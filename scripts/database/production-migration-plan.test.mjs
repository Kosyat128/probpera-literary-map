...
describe("guarded production database reconciliation", () => {
  it("keeps the exact Wells revision after backup/schema verification and before full archive preflight", () => {
    const steps = parse(workflowSource).jobs.reconcile.steps;
    const repairIndex = steps.findIndex(step => step.name === "Reconfirm main and apply the exact reviewed Wells editorial revision");
    const dependencyIndex = steps.findIndex(step => step.name === "Install the locked dependency graph for the archive release");
    const preflightIndex = steps.findIndex(step => step.name === "Run the read-only full archive preflight");
    expect(repairIndex).toBeGreaterThan(dependencyIndex);
    expect(preflightIndex).toBeGreaterThan(repairIndex);
    expect(steps[repairIndex].run).toContain("git ls-remote --exit-code origin refs/heads/main");
    expect(steps[repairIndex].run).toContain('[[ "$current_main_sha" == "$EXPECTED_MAIN_SHA" ]]');
    expect(steps[repairIndex].run).toContain("node scripts/database/repair-wells-editorial-evidence-20260913.mjs --apply");
    expect(steps[repairIndex].env.EXPECTED_MAIN_SHA).toBe("${{ inputs.expected_main_sha }}");
    expect(workflowSource).toContain("reconciliation/wells-editorial-repair-receipt.json");
    expect(workflowSource.match(/node scripts\/sync-literary-archive\.mjs --apply/gu)).toHaveLength(1);
  });
  it("builds only the reviewed ordered migration plan", () => {
    ...
    try {
      ...
      expect(manifest.migrations).toHaveLength(37);
      ...
      expect(plan).toContain("Reviewed atomic registry rotation capability is incomplete");
      expect(plan).toContain("public.prepare_literary_archive_draft_writer_reference(uuid)");
      ...
      expect(verification).toContain("public.probpera_schema_migrations) >= 37");
