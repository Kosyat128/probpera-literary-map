...
## Исправление прав публикации, 5 сентября 2026

Добавлена только миграция `20260905_article_publication_permissions.sql`:
она заменяет недоступную клиенту блокировку рабочего черновика на узкий
owner/admin RPC и направляет запись перенаправления через существующие
защищённые SEO RPC. Внешние функции публикации остаются `SECURITY INVOKER`;
RLS, CAS и запрет прямого DML к черновикам и перенаправлениям не снимаются.
Миграция проверяет предшествующий код и итоговые права внутри той же транзакции.

`ADMIN-ARTICLE-PUBLICATION-PERMISSIONS-2026-09-05` в
`scripts/stage5-baseline-registry.mjs` фиксирует рассмотренный SHA-256 миграции
и ровно три допустимых изменения: её добавление в конец плана, новый диапазон
в отчёте и ожидаемые 35 записей ledger. Проверка Stage 5 обратной точной
проекцией этих фрагментов сохраняет исходный fingerprint всех 72 файлов;
старые SHA и исторические миграции не заменяются. Любое другое изменение
остаётся под прежним контролем. Merge автоматически вызывает существующий
dispatcher по изменению planner/workflow; backup, restore drill, проверки
актуального SHA и fail-closed production verification обязательны.

Последующая проверка книжного архива отдельна от миграции админки: отказ
редакционного preflight архива после успешной schema verification не означает
отката уже подтверждённой транзакции схемы и не разрешает обходить этот preflight.
