...
1. включить `Always Use HTTPS`, чтобы HTTP-запросы к `probpera.ru` и `admin.probpera.ru` получали постоянный redirect с сохранением пути и query;
...
3. после следующей публикации убедиться, что оба `/.well-known/security.txt` доступны, затем вручную перезапустить `Audit live production security`.

Админка уже формирует защитные заголовки в Next.js. Cloudflare-настройки в репозитории не меняются и должны применяться владельцем зоны.
