...
Для автономного завершения в репозитории также есть одноразовая обёртка
`scripts/run-d-transfer-once-after-reboot.ps1`. Она сначала проверяет, что
исходник остаётся чистым опубликованным `origin/main`, на D: нет оставшихся
процессов переноса, действительно связанных с `D:\Codex`, том NTFS имеет
достаточно места, системный признак `DirtyBitSet` снят и `chkdsk D: /scan`
завершается успешно. Только
после этого запускается перенос с восстановимым архивированием неполной цели.
Журнал сохраняется на C: в `.local-ops/d-transfer-after-reboot.log`; секреты
для запуска не требуются. Задача удаляет себя только после совпадения SHA и
чистого статуса установленной D:-копии. При любой ошибке она ничего не удаляет
и сохраняется для диагностики.

Перед полным перезапуском Windows задача регистрируется из чистого уже
опубликованного репозитория в PowerShell с правами администратора:

```powershell
& '.\scripts\register-d-transfer-after-reboot.ps1'
```

Регистратор сверяет `HEAD`, `origin/main`, официальный `origin` и чистый статус,
создаёт `ProbperaCanonicalDTransferOnce` для текущего пользователя с уровнем
`Highest` и минутной задержкой после входа. При неуспехе предварительной проверки
задача остаётся на следующий вход; после подтверждённого переноса удаляет себя.
