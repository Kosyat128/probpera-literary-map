...
2. `@cf/openai/gpt-oss-120b` - независимая высокоуровневая сверка с русским оригиналом и финальная редактура до естественного литературного английского. Это Cloudflare-hosted Workers AI model, поэтому отдельный платный OpenAI API key не используется.
...
- Если первый структурированный ответ не соответствует редакционной JSON-схеме
  или меняет защищённую структуру HTML,
  выполняется один корректирующий проход по тому же исходнику и конкретным
  ошибкам полей. Проверка схемы не отключается; повторный сбой останавливает
  запись с путём проблемного поля вместо общего `Invalid input`. Если для
  пакета включён premium review, исправленный результат после этого всё равно
  проходит отдельный финальный редакторский проход.
...
- Для HTML-fallback машинной версии рядом с пустым TipTap-документом сохраняется
  упорядоченный manifest изображений (`mediaId`, `src`, `alt`, decorative). Он
  сохраняет кратность повторных изображений и не позволяет JSON/HTML незаметно
  разойтись; при следующем сохранении legacy manifest канонизируется из HTML.
...
Из центра запускаются небольшие пакеты статей, книг, биографий, стран и site-copy. Книги, биографии и страны переносят bounded cursor между пакетами, поэтому повторные запуски постепенно обходят весь допустимый архив, а не начинают каждый раз с первых записей. После фактических изменений создаётся обычный public build request.
...
CLOUDFLARE_TRANSLATION_REVIEW_MODEL=@cf/openai/gpt-oss-120b
OPENAI_PREMIUM_TRANSLATION_REVIEW=true
```

Для Workers AI request contract учитываются различия моделей: Gemma получает `max_completion_tokens`, а семейство Cloudflare-hosted `gpt-oss` - документированный `max_tokens`. Это предотвращает случайное использование короткого model-default на финальном редакторском проходе длинных материалов.
