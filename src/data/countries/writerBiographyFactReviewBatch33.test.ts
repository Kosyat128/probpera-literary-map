...
import {
  WRITER_BIOGRAPHY_FACT_REVIEW_BATCH33_REVIEWER,
  writerBiographyFactReviewBatch33,
  type WriterBiographyFactReviewRecord,
} from "./writerBiographyFactReviewBatch33";

const expectedKeys = [
  "iran:forugh_farrokhzad",
  "iran:hafez",
  "iran:mahmoud_dowlatabadi",
  "iran:omar_khayyam",
  "iran:saadi_shirazi",
  "iran:sadegh_hedayat",
  "iran:shahrnush_parsipur",
  "iran:simin_daneshvar",
  "iraq:abd_al_wahhab_al_bayati",
  "iraq:abu_nuwas",
  "iraq:al_mutanabbi",
  "iraq:badr_shakir_al_sayyab",
  "iraq:fuad_al_takarli",
  "iraq:nazik_al_malaika",
  "ireland:bram_stoker",
  "ireland:edna_obrien",
  "ireland:flann_obrien",
  "ireland:george_bernard_shaw",
  "ireland:iris_murdoch",
  "ireland:james_joyce",
  "ireland:jonathan_swift",
  "ireland:oscar_wilde",
  ...
  "israel:etgar_keret",
  "israel:orly_castel_bloom",
  "israel:s_y_agnon",
  "israel:zeruya_shalev",
  "italy:alberto_moravia",
  "italy:alessandro_baricco",
  "italy:alessandro_manzoni",
  "italy:andrea_camilleri",
  "italy:baldassare_castiglione",
  "italy:carlo_goldoni",
  "italy:cesare_beccaria",
  "italy:cesare_pavese",
] as const;
...
const reportPath = path.resolve(
  process.cwd(),
  "reports/writer-biography-fact-review-batch33.json"
);
const markdownReportPath = path.resolve(
  process.cwd(),
  "reports/writer-biography-fact-review-batch33.md"
);
...
describe("writer biography claim review batch 33", () => {
  it("pins the exact 40-key lexicographic boundary without overlap", () => {
    ...
    const keys = writerBiographyFactReviewBatch33.map((record) => record.key);
    const heldKeys = writerBiographyFactReviewBatch33
      .filter((record) => record.decision === "held")
      .map((record) => record.key);
    const applicableKeys = writerBiographyFactReviewBatch33
      .filter((record) => record.decision !== "held")
      .map((record) => record.key);
    ...
    expect(priorAssigned).toHaveLength(760);
    expect(priorAssignedSet.size).toBe(760);
    expect(pendingKeys).toHaveLength(967);
  ...
  it("pins raw source hashes, professional Russian and independent evidence", () => {
    ...
    for (const record of writerBiographyFactReviewBatch33) {
    ...
    const decisions = writerBiographyFactReviewBatch33.map(
  ...
  it("integrates applicable records through the build-only registry", () => {
    ...
    expect(runtimeIndex).not.toContain("writerBiographyFactReviewBatch33");
    expect(runtimeReviewAggregator).not.toContain(
      "writerBiographyFactReviewBatch33"
    );
    expect(buildSource).toContain("writerBiographyFactReviewBatch33");
  });
  ...
  it("keeps strict UTF-8 and JSON/Markdown reports synchronized", () => {
    const sourcePaths = [
      path.resolve(
        process.cwd(),
        "src/data/countries/writerBiographyFactReviewBatch33.ts"
      ),
      path.resolve(
        process.cwd(),
        "src/data/countries/writerBiographyFactReviewBatch33.test.ts"
      ),
      reportPath,
      markdownReportPath,
    ];
    ...
    expect(report.records).toEqual(writerBiographyFactReviewBatch33);
    ...
    expect(markdown).toContain("overlap с Batch01–32: 0");
  });
});
