...
import {
  WRITER_BIOGRAPHY_FACT_REVIEW_BATCH39_REVIEWER,
  writerBiographyFactReviewBatch39,
  type WriterBiographyFactReviewRecord,
} from "./writerBiographyFactReviewBatch39";

const expectedKeys = [
  "mali:modibo_sounkalo_keita",
  "mali:yambo_ouologuem",
  "malta:anton_manuel_caruana",
  "malta:dun_karm_psaila",
  "malta:manwel_dimech",
  "malta:oliver_friggieri",
  "malta:ruzar_briffa",
  "marshall_islands:kathy_jetnil_kijiner",
  "mauritania:ahmedou_ould_abdel_kader",
  "mauritania:hamed_ould_hamdane",
  "mauritania:mokhtar_ould_hamidoun",
  "mauritania:moussa_ould_ebnou",
  "mauritius:ananda_devi",
  "mauritius:edouard_maunick",
  "mauritius:jmg_le_clezio",
  "mauritius:khal_torabully",
  "mauritius:malcolm_de_chazal",
  "mauritius:nathacha_appanah",
  "mexico:alfonso_reyes",
  "mexico:alvaro_enrigue",
  "mexico:amparo_davila",
  "mexico:carlos_de_siguenza",
  "mexico:carlos_fuentes",
  "mexico:cristina_rivera_garza",
  "mexico:elena_garro",
  "mexico:elena_poniatowska",
  "mexico:fernanda_melchor",
  "mexico:guadalupe_nettel",
  "mexico:ignacio_manuel_altamirano",
  "mexico:javier_velasco",
  "mexico:jorge_volpi",
  "mexico:jose_emilio_pacheco",
  "mexico:jose_joaquin_fernandez_de_lizardi",
  "mexico:jose_vasconcelos",
  "mexico:juan_rulfo",
  "mexico:laura_esquivel",
  "mexico:manuel_gutierrez_najera",
  "mexico:mariano_azuela",
  "mexico:octavio_paz",
  "mexico:sergio_pitol",
] as const;

const expectedHeldKeys = ["mauritania:hamed_ould_hamdane"] as const;

const factQaPath = path.resolve(process.cwd(), "reports/writer-biography-fact-qa.json");
const reportPath = path.resolve(process.cwd(), "reports/writer-biography-fact-review-batch39.json");
const markdownReportPath = path.resolve(process.cwd(), "reports/writer-biography-fact-review-batch39.md");
...
describe("writer biography claim review batch 39", () => {
  it("pins the exact next 40-key lexicographic boundary without overlap", () => {
    ...
    const keys = writerBiographyFactReviewBatch39.map((record) => record.key);
    const heldKeys = writerBiographyFactReviewBatch39.filter((record) => record.decision === "held").map((record) => record.key);
    const applicableKeys = writerBiographyFactReviewBatch39.filter((record) => record.decision !== "held").map((record) => record.key);
  ...
  it("pins raw source hashes, professional Russian and independent evidence", () => {
    ...
    for (const record of writerBiographyFactReviewBatch39) {
      ...
      expect(record.reviewer).toBe(WRITER_BIOGRAPHY_FACT_REVIEW_BATCH39_REVIEWER);
    ...
    const decisions = writerBiographyFactReviewBatch39.map((record) => record.decision);
  ...
  it("integrates applicable records through the build-only registry", () => {
    ...
    expect(runtimeIndex).not.toContain("writerBiographyFactReviewBatch39");
    expect(runtimeReviewAggregator).not.toContain("writerBiographyFactReviewBatch39");
    expect(buildSource).toContain("writerBiographyFactReviewBatch39");
  });

  it("keeps strict UTF-8 and JSON/Markdown reports synchronized", () => {
    const sourcePaths = [
      path.resolve(process.cwd(), "src/data/countries/writerBiographyFactReviewBatch39.ts"),
      path.resolve(process.cwd(), "src/data/countries/writerBiographyFactReviewBatch39.test.ts"),
      reportPath,
      markdownReportPath,
    ];
    ...
    expect(report.summary).toEqual({ records: 40, unchanged: 0, corrected: 39, held: 1 });
    expect(report.records).toEqual(writerBiographyFactReviewBatch39);
    ...
    expect(markdown).toContain("overlap с Batch01–38: 0");
  });
});
