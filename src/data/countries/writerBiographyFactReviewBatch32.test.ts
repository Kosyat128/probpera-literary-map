...
import {
  WRITER_BIOGRAPHY_FACT_REVIEW_BATCH32_REVIEWER,
  writerBiographyFactReviewBatch32,
  type WriterBiographyFactReviewRecord,
} from "./writerBiographyFactReviewBatch32";
...
const reportPath = path.resolve(
  process.cwd(),
  "reports/writer-biography-fact-review-batch32.json"
);
const markdownReportPath = path.resolve(
  process.cwd(),
  "reports/writer-biography-fact-review-batch32.md"
);
...
describe("writer biography claim review batch 32", () => {
  it("pins the exact 40-key lexicographic boundary without overlap", () => {
    ...
    const keys = writerBiographyFactReviewBatch32.map((record) => record.key);
    const heldKeys = writerBiographyFactReviewBatch32
      .filter((record) => record.decision === "held")
      .map((record) => record.key);
    const applicableKeys = writerBiographyFactReviewBatch32
      .filter((record) => record.decision !== "held")
      .map((record) => record.key);
    ...
    expect(priorAssigned).toHaveLength(720);
    expect(priorAssignedSet.size).toBe(720);
    expect(pendingKeys).toHaveLength(1007);
  ...
  it("pins raw source hashes, professional Russian and independent evidence", () => {
    ...
    for (const record of writerBiographyFactReviewBatch32) {
    ...
    const decisions = writerBiographyFactReviewBatch32.map(
  ...
  it("integrates applicable records through the build-only registry", () => {
    ...
    expect(runtimeIndex).not.toContain("writerBiographyFactReviewBatch32");
    expect(runtimeReviewAggregator).not.toContain(
      "writerBiographyFactReviewBatch32"
    );
    expect(buildSource).toContain("writerBiographyFactReviewBatch32");
  });
  ...
  it("keeps strict UTF-8 and JSON/Markdown reports synchronized", () => {
    const sourcePaths = [
      path.resolve(
        process.cwd(),
        "src/data/countries/writerBiographyFactReviewBatch32.ts"
      ),
      path.resolve(
        process.cwd(),
        "src/data/countries/writerBiographyFactReviewBatch32.test.ts"
      ),
      reportPath,
      markdownReportPath,
    ];
    ...
    expect(report.records).toEqual(writerBiographyFactReviewBatch32);
    ...
    expect(markdown).toContain("overlap с Batch01–31: 0");
  });
});
