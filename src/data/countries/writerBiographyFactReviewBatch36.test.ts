...
import {
  WRITER_BIOGRAPHY_FACT_REVIEW_BATCH36_REVIEWER,
  writerBiographyFactReviewBatch36,
  type WriterBiographyFactReviewRecord,
} from "./writerBiographyFactReviewBatch36";

const expectedKeys = [
  "kazakhstan:olzhas_suleimenov",
  "kazakhstan:ybyrai_altynsarin",
  "kazakhstan:zhussipbek_aimauytov",
  "kenya:billy_kahora",
  "kenya:binyavanga_wainaina",
  "kenya:grace_ogot",
  "kenya:henry_ole_kulet",
  "kenya:marjorie_oludhe_macgoye",
  "kenya:meja_mwangi",
  "kenya:mukoma_wa_ngugi",
  "kenya:ngugi_wa_thiongo",
  "kenya:peter_kimani",
  "kenya:yvonne_adhiambo_owuor",
  "kiribati:teresia_teaiwa",
  "kosovo:adem_demaci",
  "kosovo:ali_podrimja",
  "kosovo:flora_brovina",
  "kosovo:mehmet_kraja",
  "kosovo:rexhep_qosja",
  "kuwait:ahmad_al_adwani",
  "kuwait:fahad_al_askar",
  "kuwait:ismail_fahd_ismail",
  "kuwait:laila_al_othman",
  "kuwait:taleb_al_refai",
  "kyrgyzstan:aaly_tokombaev",
  "kyrgyzstan:chyngyz_aitmatov",
  "kyrgyzstan:kalyk_akiev",
  "kyrgyzstan:kasym_tynystanov",
  "kyrgyzstan:musa_jangaziev",
  "kyrgyzstan:suranbay_eraliev",
  "kyrgyzstan:togolok_moldo",
  "kyrgyzstan:tologon_kasymbekov",
  "kyrgyzstan:tugolbai_sydykbekov",
  "laos:outhine_bounyavong",
  "laos:phoumi_vongvichit",
  "laos:visuth_phommasane",
  "latvia:andrejs_upits",
  "latvia:aspazija",
  "latvia:janis_jaunsudrabins",
  "latvia:karlis_skalbe",
] as const;
...
const reportPath = path.resolve(
  process.cwd(),
  "reports/writer-biography-fact-review-batch36.json"
);
const markdownReportPath = path.resolve(
  process.cwd(),
  "reports/writer-biography-fact-review-batch36.md"
);
...
describe("writer biography claim review batch 36", () => {
  it("pins the exact 40-key lexicographic boundary without overlap", () => {
    ...
    const keys = writerBiographyFactReviewBatch36.map((record) => record.key);
    const heldKeys = writerBiographyFactReviewBatch36
      .filter((record) => record.decision === "held")
      .map((record) => record.key);
    const applicableKeys = writerBiographyFactReviewBatch36
      .filter((record) => record.decision !== "held")
      .map((record) => record.key);
    ...
    expect(frozenReviewQueueKeys).toHaveLength(1699);
    expect(new Set(frozenReviewQueueKeys).size).toBe(1699);
    ...
    expect(priorAssigned).toHaveLength(880);
    expect(priorAssignedSet.size).toBe(880);
    expect(pendingKeys).toHaveLength(854);
  ...
  it("pins raw source hashes, professional Russian and independent evidence", () => {
    ...
    for (const record of writerBiographyFactReviewBatch36) {
    ...
    const decisions = writerBiographyFactReviewBatch36.map(
  ...
  it("integrates applicable records through the build-only registry", () => {
    ...
    expect(runtimeIndex).not.toContain("writerBiographyFactReviewBatch36");
    expect(runtimeReviewAggregator).not.toContain(
      "writerBiographyFactReviewBatch36"
    );
    expect(buildSource).toContain("writerBiographyFactReviewBatch36");
  });
  ...
  it("keeps strict UTF-8 and JSON/Markdown reports synchronized", () => {
    const sourcePaths = [
      path.resolve(
        process.cwd(),
        "src/data/countries/writerBiographyFactReviewBatch36.ts"
      ),
      path.resolve(
        process.cwd(),
        "src/data/countries/writerBiographyFactReviewBatch36.test.ts"
      ),
      reportPath,
      markdownReportPath,
    ];
    ...
    expect(report.records).toEqual(writerBiographyFactReviewBatch36);
    ...
    expect(markdown).toContain("overlap с Batch01–35: 0");
  });
});
