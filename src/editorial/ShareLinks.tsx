...
export default function ShareLinks({ url, title, variant = "reader" }: Props) {
  ...
  return (
    <div
      className={`share-links share-links--${variant}`}
      role="group"
      aria-label={`${t("Поделиться материалом")} “${title}”`}
    >
      <span>{t("Поделиться")}</span>
      <div className="share-links__controls">
        {shareItems.map((item) => (
          <a
