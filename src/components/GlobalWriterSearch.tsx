...
export default function GlobalWriterSearch({ onWriterSelect }: GlobalWriterSearchProps) {
  ...
  return (
    <div style={{ padding: "15px", background: "#FFF8EE", borderRadius: "16px" }}>
      ...
      <select value={country} onChange={(event) => setCountry(event.target.value)}><option value="">Все страны</option>{countryList.map((item) => <option key={item}>{item}</option>)}</select>
      <select value={genre} onChange={(event) => setGenre(event.target.value)}><option value="">Все жанры</option>{genres.map((item) => <option key={item}>{item}</option>)}</select>
      <select value={language} onChange={(event) => setLanguage(event.target.value)}><option value="">Все языки</option>{languages.map((item) => <option key={item}>{item}</option>)}</select>
      <select value={period} onChange={(event) => setPeriod(event.target.value)}><option value="">Все эпохи</option>{periods.map((item) => <option key={item}>{item}</option>)}</select>
      <select value={award} onChange={(event) => setAward(event.target.value)}><option value="">Все премии</option>{awards.map((item) => <option key={item}>{item}</option>)}</select>
      {hasFilters && <button onClick={clearFilters}>✖ Очистить фильтры</button>}
