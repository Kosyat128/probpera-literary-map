...
function GlobeSurface(){return <Sphere args={[0.723,128,128]}><meshStandardMaterial color="#31476B" transparent opacity={.32} roughness={1}/></Sphere>}
function GlobeGrid(){return <Sphere args={[0.726,96,96]}><meshStandardMaterial color="#E97824" wireframe transparent opacity={.05}/></Sphere>}
...
function LiteraryRing(){return <Sphere args={[0.91,64,64]}><meshBasicMaterial color="#E97824" transparent opacity={.02}/></Sphere>}
...
export default function LiteraryGlobe({onCountrySelect}:Props){
 return <div style={{width:'100%',height:'560px',background:'radial-gradient(circle,#35205F,#1F103D)',borderRadius:18,overflow:'hidden'}}>
  <Canvas camera={{position:[0,0,3.45],fov:35}}>
   ...
   <LiteraryRing/><Glow/><GlobeBase/><GlobeSurface/><GlobeGrid/><Atmosphere/>
   <LiteraryMarkers onCountrySelect={onCountrySelect}/>
   <OrbitControls enableZoom enablePan={false} autoRotate autoRotateSpeed={0.08} minDistance={2.3} maxDistance={5}/>
  </Canvas>
 </div>
}
