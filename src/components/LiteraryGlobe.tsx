  ...
  const markers = useMemo(() => countries
    ...
    .map((country) => {
      ...
      return {
        ...
        color: country.writers.length > 20 ? "#E97824" : "#35205F",
      };
    }), []);
    ...
    {markers.map((point) => (
      <group key={point.name} position={point.position}>
        <mesh onClick={() => onCountrySelect?.(point.name)}>
          <sphereGeometry args={[0.055 + Math.min(point.count / 500, 0.04), 32, 32]} />
          <meshStandardMaterial color={point.color} emissive={point.color} emissiveIntensity={1} />
        </mesh>
...
export default function LiteraryGlobe({ onCountrySelect }: Props) {
  return <div style={{ width: "100%", height: "760px", background: "#1F103D", borderRadius: 18, overflow: "hidden" }}>
