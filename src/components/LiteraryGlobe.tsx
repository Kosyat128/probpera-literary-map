...
function LiteraryMarkers({onCountrySelect}:Props){
 ...
 <mesh onClick={()=>{setActive(p.name);onCountrySelect?.(p.name)}} onPointerOver={()=>setHovered(p.name)} onPointerOut={()=>setHovered(null)}><sphereGeometry args={[focus?p.size*2.5:p.size,32,32]}/><meshStandardMaterial color={focus?"#FFD166":p.color} emissive={p.color} emissiveIntensity={focus?5:1.5}/></mesh>
...
function GlobeContinents(){return <Sphere args={[0.729,128,128]}><meshStandardMaterial color="#8B5CF6" transparent opacity={.08} roughness={1}/></Sphere>}
function GlobeGrid(){return <Sphere args={[0.732,96,96]}><meshStandardMaterial color="#E97824" wireframe transparent opacity={.035}/></Sphere>}
function Atmosphere(){return <Sphere args={[0.82,64,64]}><meshBasicMaterial color="#8B5CF6" transparent opacity={.16}/></Sphere>}
function LiteraryRegions(){return <Sphere args={[0.738,64,64]}><meshBasicMaterial color="#E97824" transparent opacity={.025}/></Sphere>}
...
export default function LiteraryGlobe({onCountrySelect}:Props){
 ...
 <GlobeBase/><GlobeSurface/><GlobeContinents/><LiteraryRegions/><GlobeGrid/><Atmosphere/>
