...
type TypographyOverrideRow = SiteTypographyOverride & {
  id: string;
  casVersion: number;
...
function normalizeOverride(value: unknown): TypographyOverrideRow | null {
  ...
  try {
    ...
    const draftSettings = readSiteTypographyProperties(source.draft_settings);
    return {
      id,
      casVersion,
      ...target,
...
function normalizeRevision(value: unknown): TypographyRevisionRow | null {
  const source = record(value);
  if (!source) return null;
...
function typographyStatus(row: TypographyOverrideRow) {
...
function TypographyEditor({
  ...
  fonts: FontAssetRow[];
  selected: TypographyOverrideRow | null;
  canManage: boolean;
}) {
...
export default async function SiteTypographyPage({
  ...
  const fonts = (assetResult.data || []) as FontAssetRow[];
  ...
  const selected = query.override ? overrideById.get(query.override) || null : null;
  const schemaError = assetResult.error || overrideResult.error || revisionResult.error;
  ...
  return (
    <div className={styles.page}>
      ...
      {query.error && <p className={styles.message} role="alert">{query.error}</p>}
      {query.saved && <p className={`${styles.message} ${styles.success}`} role="status">Черновик сохранён.</p>}
      {query.published && <p className={`${styles.message} ${styles.success}`} role="status">Настройка опубликована.</p>}
      {query.restored && <p className={`${styles.message} ${styles.success}`} role="status">Версия восстановлена и опубликована.</p>}
      {query.reset && <p className={`${styles.message} ${styles.success}`} role="status">Настройка сброшена, снова действует наследование.</p>}
      {query.archived && <p className={`${styles.message} ${styles.success}`} role="status">Шрифт перемещён в архив.</p>}
      {schemaError && (
                ...
                {fonts.map((font) => (
                  <li className={styles.asset} key={font.id}>
                    ...
                    <small>
                      ...
                      {safeLicenseUrl(font.license_url) && (
                        <>
                          {" · "}
                          <a
                            className={styles.licenseLink}
                            href={safeLicenseUrl(font.license_url) || undefined}
                            target="_blank"
                            rel="noreferrer"
                          >
                            открыть условия ↗
                ...
                {revisions.map((revision) => {
                  ...
                  return (
                    <li className={styles.revision} key={revision.id}>
                      ...
                      <small>{revision.createdAt ? formatDate(revision.createdAt, true) : "Дата не указана"}</small>
                      {canManage && current && (
