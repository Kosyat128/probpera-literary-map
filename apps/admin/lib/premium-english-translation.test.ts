...
describe("premium English translation", () => {
  it("uses a dedicated translator pass followed by a reviewer pass", async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce(response("First draft", "resp_translate", "req_translate"))
      .mockResolvedValueOnce(response("Final native English", "resp_review", "req_review")) as unknown as typeof fetch;
